...
import { FillInBlankQuestion } from '../../../quiz-engine/schema';
...
export function FillInBlankForm({
  ...
  question: FillInBlankQuestion | null;
  ...
  const handleSave = () => {
    ...
    if (!text.includes('________')) {
      alert('Question text must include "________" to indicate the blank');
      return;
    }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>
          Question Text * (use ________ for the blank)
        </label>
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          rows={3}
          placeholder="The capital of France is ________."
          style={{
            width: '100%',
            padding: '0.5rem',
            backgroundColor: theme.secondary,
        ...
        />
        {!text.includes('________') && text.trim() && (
          <div style={{ color: theme.warning || '#f59e0b', fontSize: '0.85rem', marginTop: '0.5rem' }}>
            ⚠️ Don't forget to include ________ where the blank should be
          </div>
        )}
      </div>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <div style={{ fontSize: '0.75rem', marginTop: '0.5rem', opacity: 0.7 }}>
          Add multiple acceptable answers (e.g., "Paris", "paris"). Fuzzy matching will handle minor typos.
        </div>
      </div>
